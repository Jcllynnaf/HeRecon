...
# --- Configuration ---
SHODAN_API_KEY = "ojBfpbbyvb1aXZADeMznWOS9mmPtFtsl" # Replace with your API Key
...
def shodan_lookup(ip, screen=None):
    ...
    if not SHODAN_API_KEY or SHODAN_API_KEY == "ojBfpbbyvb1aXZADeMznWOS9mmPtFtsl": return None # Check against the actual placeholder value
...
░▓█▒░██▓░▒████▒░██▓ ▒██▒░▒████▒▒ ▓███▀ ░░ ████▓▒░▒██░   ▓██░
 ...
 HeRecon
 Author : ByteLoneCode
 Contact : https://instagram.com/bytelonecode
 ============================================
    """
...
def run_osint_app(screen):
    ...
    curses.init_pair(COLOR_PAIR_HEADER, curses.COLOR_BLUE, -1) # Header/Art
    curses.init_pair(COLOR_PAIR_INFO, curses.COLOR_BLUE, -1)   # Domain/Table Info
    curses.init_pair(COLOR_PAIR_PROMPT, curses.COLOR_BLUE, -1) # Input Prompt
    curses.init_pair(COLOR_PAIR_MENU_NUMBER, curses.COLOR_BLUE, -1) # Menu Numbers
    curses.init_pair(COLOR_PAIR_MENU_TEXT, curses.COLOR_BLUE, -1) # Menu Text & Table Items
    # Status Bar: RED Text, Default Background (-1)
    curses.init_pair(COLOR_PAIR_STATUS, curses.COLOR_BLUE, -1)
    ...
    while True:
        ...
        try:
            if key == ord('1'): # Set Domain
                ...
                if not SHODAN_API_KEY or SHODAN_API_KEY == "ojBfpbbyvb1aXZADeMznWOS9mmPtFtsl": update_status(screen, "[-] Shodan API Key not configured or is placeholder."); time.sleep(3); continue # RED Status
...
if __name__ == "__main__":
    ...
    if not SHODAN_API_KEY or SHODAN_API_KEY == "ojBfpbbyvb1aXZADeMznWOS9mmPtFtsl": print("WARNING: SHODAN_API_KEY is not set or is the default placeholder!"); api_key_ok = False
